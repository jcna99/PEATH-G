...
__device__ __host__
#endif
float test_rand(BlockType block, uint phi, uint indvi, uint pos, uint gen)
{
	uint x = block.start_frag*phi + block.end_frag*indvi + block.length*pos + gen;
	x = (x + block.start_frag + block.end_frag + block.end_frag + phi + indvi + pos + gen) % 100;
	...
	const BlockType *d_blocks, const uint NumPos, const uint NumBlk)
{
	...
	for (blockIdx.z = 0; blockIdx.z < gridDim.z; ++blockIdx.z) // 1
...
#endif
		{
			...
			//uint id = (blockIdx.x * gridDim.y + blockIdx.y)*blockDim.x + threadIdx.x;
			...
			for (uint indv = 0; indv < POPSIZE; ++indv) {
				...
				for (uint i = threadIdx.x; i < block.length; i += blockDim.x) {
					...
					//if (test_rand(block, blockIdx.x, indv, i, 0) < 0.5)
...
#endif
		{
			...
			for (uint indv = (POPSIZE - OFFSIZE); indv < POPSIZE; ++indv) {
				...
				for (uint i = threadIdx.x; i < block.length; i += blockDim.x) { // position inside of haplotype block
					...
					//if (test_rand(block, blockIdx.x, indv, i, g_iter) < prob)
	...
	IndvDType *d_Population, const uint NumBlk)
{
	...
	for (blockIdx.z = 0; blockIdx.z < gridDim.z; ++blockIdx.z) // 1
...
#endif
		{
			...
			//std::sort(Population, Population + POPSIZE, compare_sumD_val);
	...
	const uint NumPos, const uint NumBlk, const uint MaxBlkLen, const uint phasing_iter)
{
	...
	//dim3 gDim_PBP = { phasing_iter, NumBlk, POPSIZE };
#if __CUDA__ == 0
	...
	while (g_iter--) {
			...
			//dim3 gDim_PBO = { phasing_iter, NumBlk, OFFSIZE };
#if __CUDA__ == 0
...
/*
#if __CUDA__ == 1
// this function is run by MULTI thread, but actually by ONE thread
__device__
#endif
	...
	const FragsForPosType *d_FragsForPos, const uint NumFrag, const uint phasing_iter)
{
	...
	do {		// while finding better solution
		...
		for (threadIdx.x = 0; threadIdx.x < blockDim.x; ++threadIdx.x) {	// position is grouped by thread
			...
			if (thIdx == 0) {
				for (uint i = 0; i < block.length; i++) {	// for each position in block

															// calculating the w-MEC value assuming that haplo.seq[i] is toggled
					tmpSum = calc_sumD_single_tog(haplo_sumD, haplo_seq, DFrag, block, i, true,
						d_AlleleData, d_QualData, d_SubFragments, d_Fragments, d_FragsForPos); // DFrag array & haplo are updated
						// To do. parallize using multiple thread ######################
	...
	for (blockIdx.z = 0; blockIdx.z < gridDim.z; ++blockIdx.z) // 1
#endif
		{
			...
			while (tog_iter--) {
				...
				//			imp += range_switch_toggling(			// slower
